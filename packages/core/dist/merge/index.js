 ...
 */
export function strictMerge(target, source, options = {}) {
    const changedPaths = [];
    const discardedKeys = [];
    const trackDiscarded = options.trackDiscarded ?? false;
    const result = mergeValue(target, source, "", changedPaths, discardedKeys, trackDiscarded);
    return {
        data: result.value,
...
function mergeValue(target, source, path, changedPaths, discardedKeys, trackDiscarded) {
    ...
    if (Array.isArray(target)) {
        return mergeArray(target, source, path, changedPaths, discardedKeys, trackDiscarded);
    }
    // Handle objects
    if (isPlainObject(target) && isPlainObject(source)) {
        return mergeObject(target, source, path, changedPaths, discardedKeys, trackDiscarded);
    }
    // Handle primitives
    return mergePrimitive(target, source, path, changedPaths);
}
function mergeObject(target, source, path, changedPaths, discardedKeys, trackDiscarded) {
    ...
    for (const key of Object.keys(target)) {
        ...
        const result = mergeValue(targetValue, sourceValue, childPath, changedPaths, discardedKeys, trackDiscarded);
        if (result.changed) {
            anyChanged = true;
...
function mergeArray(target, source, path, changedPaths, discardedKeys, trackDiscarded) {
    ...
    for (let i = 0; i < maxLen; i++) {
        ...
        if (i < target.length && i < source.length) {
            ...
            const result = mergeValue(target[i], source[i], childPath, changedPaths, discardedKeys, trackDiscarded);
            if (result.changed) {
                anyChanged = true;
...
function mergePrimitive(target, source, path, changedPaths) {
 ...
 */
function coerceType(target, source) {
 ...
 */
export function applyParsedValue(target, path, value) {
    const changedPaths = [];
...
function applyAtPath(target, segments, index, value, changedPaths) {
    ...
    if (segment.type === "key") {
        ...
        // Recurse into child
        const childResult = applyAtPath(obj[key], segments, index + 1, value, changedPaths);
        ...
        // Recurse into child
        const childResult = applyAtPath(arr[idx], segments, index + 1, value, changedPaths);
        if (!childResult.changed) {
            return { value: target, changed: false };
        }
 ...
 */
export function trimSkeleton(data, actualArrayLengths) {
    if (actualArrayLengths.size === 0) {
        return data;
    }
    return trimValue(data, "", actualArrayLengths);
}
function trimValue(value, path, lengths) {
